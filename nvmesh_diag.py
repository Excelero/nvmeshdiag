...
EXCELERO_MLXCONFIG_DB = "/etc/opt/NVMesh/Excelero_mlxconfig.db"
...
CMD_GET_ONE_QP = "mlxconfig -d %s -b %s query ONE_QP_PER_RECOVERY | grep ONE_QP_PER_RECOVERY"
...
def get_ip_info():
    try:
        ...
        print get_command_return_code("ip -4 a s")
        print "\n", get_command_return_code("ip -s link")
        output.write(get_command_return_code("ip -s link"))
        print("Done.")
    except Exception, e:
        logging.debug(e.message)
        print e.message
