...
func parseLSPCI(s string) {
	...
	for _, device := range slcPCIdevices {
		...
		if strings.Contains(strings.ToLower(string(device)), "non-volatile memory controller") {
			slcNVMEdrives = append(slcNVMEdrives, device)
		}
	}
	...
	fmt.Println(formatBoldWhite("\nNVMe Drives Found: " + strconv.Itoa(len(slcNVMEdrives))))
	for _, n := range slcNVMEdrives {
		parsePCIdeviceDetail(strings.Split(n, "\n"))
	}
}
